...
const ContactPage: NextPage<ContactPageProps> = ({
  ...
  // TODO: test if discord api doesnt work it shoul not show but github should show
  return (
    <div className="pt-12">
      ...
      <div>
        {discordInfo && (
          <div className={`${contactRowClassNames}`}>
            <div className={`${flexItemsCenter}`}>
              ...
              <span className="ml-4">{discordInfo.contactProvider}</span>
            </div>
            <div className="font-semibold hover:scale-125">
          ...
          <div className={`${contactRowClassNames}`}>
            <div className={`${flexItemsCenter}`}>
              ...
              <Link href={githubInfo.link}>
                <a rel="noopener noreferrer" target="_blank" className="ml-4">
        ...
        {/*   {contactInfo &&
          contactInfo.map((contact, idx) => (
            <div className="flex justify-between items-center py-6 border-b-[3px] border-dashed border-red-100">
              <div key={idx} className="flex items-center ">
                {contact.contactProvider === ContactType.GITHUB && (
                  <i className="devicon-github-original text-5xl"></i>
                )}
                {contact.contactProvider === ContactType.DISCORD && (
                  <svg
                    stroke="currentColor"
                    fill="currentColor"
                    stroke-width="0"
                ...
                <span className="ml-4"> {contact.contactProvider}</span>
              </div>
              <div className="font-semibold hover:scale-125">
                {contact.account}
              </div>
